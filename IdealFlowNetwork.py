...
def virtualMemory(limit):
    '''
    wait for memory usage under limit to run next step
    '''
    vm = psutil.virtual_memory()[2]<limit
    while vm == False:
        sleep(2)
        vm = psutil.virtual_memory()[2]<limit
...
def steadyStateMC(S,kappa=1):
    ...
    kappa is the total of Markov vector with manual computation
    '''
    [m,n] = S.shape
    if m == n :
        X = np.zeros((m,n))
        for a in range(m):
            for b in range(n):
                if a == b:
                    ...
                    X[a,b] = S[b,a]
        X = np.vstack([X,np.ones((1,n))*kappa])
        del S
    return (np.reshape(pseudoinverse(X)[:,-1],(n,1)))"""
...
def steadyStateMC(S,kappa=1):
    ...
    if m==n:
        ...
        Xp=pseudoinverse(X)      # Moore-Penrose inverse
...
def idealFlow(S,pi):
    ...
    '''
    [m,n]=S.shape
    return np.multiply(np.dot(pi,np.ones((1,n))),S)
